;; DeFi Marketplace Template
;; This Clarity smart contract provides a comprehensive decentralized marketplace framework on the Stacks blockchain:
;; - Secure peer-to-peer product listings
;; - Trustless escrow and transaction management
;; - Built-in dispute resolution mechanism
;; - Reputation tracking for buyers and sellers
;; 
;; Key Features:
;; - Permissionless listing and purchasing
...
(define-constant err-inactive-listing (err u103))
(define-constant err-insufficient-inventory (err u104))
(define-constant err-locked-listing (err u105))
(define-constant err-buyer-permission-required (err u106))
(define-constant err-seller-permission-required (err u107))
(define-constant err-already-confirmed-transaction (err u108))
(define-constant err-already-refunded-transaction (err u109))
(define-constant err-already-disputed-transaction (err u110))
(define-constant err-dispute-resolution-expired (err u111))
(define-constant err-active-dispute-period (err u112))
(define-constant err-invalid-transaction-rating (err u113))
(define-constant err-escrow-incomplete (err u114))
